...
- 原生Python中无Arrays类型，要使用Arrays，必须导入一个库，如NumPy

- list与Arrays 区别

  | 特性         | `list`           | `array`               |
  | :----------- | :--------------- | :-------------------- |
  | **是否内置** | ✅ Python 内置    | ❌ 需要 `import array` |
  | **存储类型** | 可以混合类型     | 只能存储相同类型      |
  | **内存占用** | 较大（存储灵活） | 较小（优化存储）      |
  | **性能**     | 较慢（动态类型） | 更快（C 数组实现）    |
  | **适用场景** | 通用数据存储     | 数值计算、大数据存储  |

- **numpy.array  ** 如果需要进行**数值计算**（矩阵运算、科学计算），`array` 仍然不够高效，推荐使用 **`numpy.array`**：
